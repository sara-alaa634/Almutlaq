...
import CasePaymentTable from "./CasePaymentTable";
import Button from "../../../SharedComponents/Buttons/Button";
import Search from "../../../SharedComponents/Search";
import { useTranslation } from "react-i18next";
import CasePaymentModal from "./CasePaymentModal";
import CasePaymentStatistics from "./CasePaymentStatistics";
...
const CasePayemnt = ({ caseID }) => {
  ...
  const { fetchCasePayments, removeCasePayment, fetchCasePaymentsTotal } =
    useCases();
  ...
  useEffect(() => {
    const handleTotalPayments = async () => {
      const data = await fetchCasePaymentsTotal(caseID);
      setTotalPaymentsData(data);
    };
    handleTotalPayments();
  }, []);
  ...
  const fetchFilterdData = async (page, paginationState = false) => {
    ...
    await handlePagination(fetchCasePayments, filterValue);
    setLoading(false);
  };
    ...
    <div className="card">
      <div className="card-body">
        ...
        <section className="table-wrapper mt-4">
          {totalPaymentsData && (
            <CasePaymentStatistics totalPaymentsData={totalPaymentsData} />
          )}
          <div className="table-responsive-xl">
            <CasePaymentTable
              paginationData={paginationData}
              setPaginationData={setPaginationData}
              onDelete={removeCasePayment}
