...
import UsersSingleTable from "./UsersSingleTable";
...
const UsersTables = ({
  ...
  useEffect(() => {
    //   console.log("fetch data from all parametes");
    if (selectUserStatus === "") {
      setCurrentPage(1);
      fetchFilterdData(1);
    } else {
  ...
  const handleExport = async () => {
    ...
    const exportData = await fetchFilteredUsers("user");
